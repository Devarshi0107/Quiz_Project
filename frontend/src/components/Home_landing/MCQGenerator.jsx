...
// 1) above don e without storrinng timee date
//this is for time date batchh rolll number and techer name storing 
...
const MCQGenerator = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setQuestions(json.questions); // Assuming the uploaded JSON has a "questions" array
      } catch (error) {
  ...
  const createTest = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/save-quiz', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          filename,
          teacher_name: "Teacher's Name", // Replace with actual teacher's name
          exam_name: examName,
