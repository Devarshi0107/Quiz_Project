...
// Mock exam details for header info (update these as needed)
const examData = [
  {
    id: '67aec3957bc7c0ee0f7a79ba', // Ensure this matches the quizId from your API
    name: 'Advanced Mathematics',
    date: 'February 20, 2024',
    totalMarks: 100,
    leaderboard: [] // Will be replaced with API data
...
const TeacherExamLeaderboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchLeaderboard = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/leaderboard/getleaderboard/${quizId}`);
        // API returns an object with a "leaderboard" key containing a "rankings" array.
        // Map the rankings array to the format: { rank, name, marks }
        const apiRankings = response.data.leaderboard.rankings.map((item, index) => ({
          rank: index + 1,
          name: item.rollNo, // Using rollNo as the student name (adjust if needed)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <TeacherNavbar
        toggleSidebar={() => setIsSidebarOpen((prev) => !prev)}
        teacher={{ name: 'John Smith' }} // Example teacher data
        isLoggedIn={true}
        userType="teacher"
      />
      <TeacherSidebar
